...
export default function Settings({ jobs, syncUserId, onMergeDuplicates, onUpdateJob, initialTab }) {
  ...
  // New layout is ON by default; this flag is a "revert to old design" switch.
...
{JSON.stringify(calDiag, null, 2)}
              ...
              <Card title="🧪 Expérimental" subtitle="Fonctionnalités en cours d'évaluation — peuvent changer ou disparaître.">
                ...
                <Row label="Ancienne interface (classique)" hint="La nouvelle interface (Focus + Liste : barre latérale de navigation + panneau détail latéral) est désormais activée par défaut. Activez cette option pour revenir à l'ancienne interface avec en-tête supérieur.">
                  <button
                    onClick={() => {
                      const next = !legacyLayoutFlag
                      setFlag(FLAGS.LEGACY_LAYOUT, next)
                      setLegacyLayoutFlag(next)
                    }}
                    className={`px-4 py-2 rounded-lg font-medium transition-all ${
                      legacyLayoutFlag
                        ? 'bg-green-500 text-white hover:bg-green-600'
                  ...
                  >
                    {legacyLayoutFlag ? '✓ Activée' : 'Désactivée'}
                  </button>
                </Row>
