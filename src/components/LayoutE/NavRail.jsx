  ...
  return (
    <aside className="hidden md:flex fixed top-0 left-0 bottom-0 z-30 w-[220px] flex-col bg-white border-r border-gray-100 shadow-[1px_0_8px_0_rgba(0,0,0,0.04)]">
      ...
      <div className="px-3 py-3 border-t border-gray-100 flex flex-col gap-1 shrink-0">
        ...
        {/* Sign out (Supabase identity). The legacy top-header sign-out is
            md:hidden under this layout, so the rail is the only desktop surface
            for it. Rendered only when a handler is passed (i.e. a session). */}
        {onSignOut && (
          <button
            onClick={onSignOut}
            className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-gray-500 hover:bg-red-50 hover:text-red-600 transition-colors"
          >
